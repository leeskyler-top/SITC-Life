    ...
    <a-row type="flex" justify="space-around" align="middle" :gutter="[8,8]">
      ...
      <a-col :lg="{span: 8}" :md="{span: 24}" :sm="{span: 24}" :xs="{span: 24}">
        <a-card title="值班管理" :style="{minHeight: '220px'}">
          <a-row :gutter="[24,24]">
            <a-col align="middle" :span="['部长', '副部长', '部门负责人'].includes(user_position) || is_admin === 'true' ? 8 : 24">
            ...
            <a-col align="middle" :span="['部长', '副部长', '部门负责人'].includes(user_position) || is_admin === 'true' ? 8 : 24" v-if="['部长', '副部长', '部门负责人'].includes(user_position) || is_admin === 'true'">
